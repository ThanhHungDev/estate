    ...
    <form class="row js-validate-form page-edit-option" action="{{ Route('ADMIN_SAVE_OPTION') }}" method="POST">
        ...
        <div class="col-md-8" id="sortable">
            ...
            <div class="row block-content js-group-option">
                ...
                <div class="col-12 bg-color-white shadows-1 px-3 py-3">
                    <div class="row py-1">
                        <div class="col-6" id="js-input-key-option-0">
                            <input name="key[]" type="text" required placeholder="nhập key vào" />
                        </div>
                        <div class="col-6 js-clone-select-option">
                            <select id="select2-0" name="type[]" class="js__single-select js__select_option_type_format" onchange="listenChangeTypeOption(this)">
                                @foreach(Config::get('constant.TYPE_OPTION') as $key => $type__option)
                                <option type="type.0-1" value="{{ $type__option }}">{{ $key }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="py-1 js-clone-value-option">
                        <input name="value_text[]" type="text" value="{{ old('value_text.0', @$option->value_text) }}" />
                        <textarea id="ckeditor-0" name="value_html[]" class="h-100 d-none">{{ old('value_html.0', @$option->value_html) }}</textarea>
                    </div>
                </div>
            </div>
            @endif
