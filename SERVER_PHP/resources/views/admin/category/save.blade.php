...
@section('javascripts')
    ...
    <script src="{{ asset('js/admin/validate.category.js') }}"></script>
    <script src="{{ asset('js/admin/app.js') }}"></script>
    
@endsection


@section('page_title', $category->id ? 'chỉnh sửa danh mục' : 'thêm mới danh mục' )
...
<div class="page__category admin-main-content">
    ...
    <form class="row js-validate-form" action="{{ Route('ADMIN_SAVE_CATEGORY', ['id' => $category->id]) }}" method="POST">
        {!! csrf_field() !!}
        <input type="hidden" name="_slug_old" value="{{ $category->slug }}">
        <div class="col-md-8">
            <div class="row block-content">
                <div class="js-parent__create-slug col-12 bg-color-white shadows-1 px-3 py-3">
                    <h2 class="title">tên category</h2>
                    <div class="input-control-link">
                        <input class="jquery__append-out" name="name" type="text" value="{{ old('name', $category->name ) }}" onblur="isExistSlug(this.value)" />
                    </div>
                    <div class="input-control-link js-input-control">
                        <input class="mt-2 jquery__append-out" name="slug" type="text" value="{{ old('slug', $category->slug ) }}" readonly onblur="isExistSlug(this.value)"/>
            ...
            <div class="row block-content">
                <div class="col-12 bg-color-white shadows-1 px-3 py-3">
                    ...
                    <textarea  class="height-80px" name="excerpt" cols="30" rows="10">{{ old('excerpt', $category->excerpt) }}</textarea>
                </div>
            </div>
            <div class="row block-content">
                <div class="col-12 bg-color-white shadows-1 px-3 py-3">
                    <h2 class="title">content</h2>
                    <textarea name="content" id="editor1" class="h-100">{{ old('content', $category->content) }}</textarea>
                </div>
            </div>
            <div class="row block-content">
                <div class="col-12 bg-color-white shadows-1 px-3 py-3">
                ...
                <div class="col-12 bg-color-white shadows-1 px-3 py-3">
                    ...
                    <div class="position-relative wrapper__selectImageWithCKFinder type-select-ckfinder__inline">
                        <input name="image_seo" class="img__outputCKFinder jquery__append-out" type="text" 
                            ...
                            value="{{ old('image_seo', $category->image_seo) }}" />
            ...
            <div class="row block-content">
                <div class="col-12 bg-color-white shadows-1 px-3 py-3">
                    ...
                    <textarea class="height-80px" name="description_seo" cols="30" rows="10">{{ old('description_seo', $category->description_seo) }}</textarea>
                </div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="row block-content">
                <div class="col-12 bg-color-white shadows-1 px-3 py-3">
                    <section class="pb-4">
                        <h2 class="title text-center">bấm lưu mới category</h2>
            ...
            <div class="row block-content">
                <div class="col-12 bg-color-white shadows-1 px-3 py-3">
                    <section class="pb-4">
                        <h2 class="title text-center">chọn user cho category</h2>
                        <select name="user_id" class="js__single-select">
                            
                            @isset($users)
                                @foreach ($users as $user)
                                <option @if(old('user_id', $category->user_id) == $user->id) {{ 'selected' }} @endif
                    ...
                    <section class="pb-4 wrapper__selectImageWithCKFinder">
                        ...
                        <div class="group-control-img-ckfinder">
                            <input name="background" class="img__outputCKFinder thumbnail-topic mb-2" 
                                onblur="showImage__InputCKFinder( this.value, this )"
                                onclick="this.setSelectionRange(0, this.value.length)"
                                type="text" value="{{ old('background', $category->background) }}" />
                        </div>
                    </section>
                </div>
            </div>
