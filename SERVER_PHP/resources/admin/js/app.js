  ...
  window.showlstImage__InputCKFinder = function (imageSrcs, btnOrInputDom) {
    ...
    for (var posShowlstImage = 0; posShowlstImage < imageSrcs.length; posShowlstImage++) {
      ...
      var imgImagesInput           = document.createElement('img')
          ...
          imgImagesInput.onerror   = function () { this.src = '/images/failed.jpg' }
      /// 
    ...
    groupJQUERY.append( groupImagesResult )
...
function resetSelect2Location( dom, data ){
    ...
    dom.select2("destroy")
    runSelect2SingleByOption(dom, { data : data })
    dom.trigger('change')
}
...
window.selectDistrictCommuneAdmin = function ( districtValue, communeValue){ 
    ...
    if( districtValue == 0 ){
        ...
        .then(function(res) {
            ...
            var COMMUNES = res.data
            var communesInDistrict = [{ id: 0, text: optionFirstCommuneText }].concat(filterLocation(COMMUNES, 'district', districtValue ))
...
window.calcTotalPriceProduct = function(){
    ...
    if(window.CONFIG_APP.UNIT.PRICE.DEFAULT.VALUE == unitPrice ){
        ...
        $output.html(`${formatNumberToDot(price * square, 'VND')} - ${DocTienBangChu(price * square)}`)
    }
}
